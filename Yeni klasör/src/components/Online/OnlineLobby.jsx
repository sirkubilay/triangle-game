...
export default function OnlineLobby() {
  ...
  const {
    ...
    createRoom, joinRoom, handlePointClick, requestRestart, leave,
  } = useOnlineGame();
          ...
          <>
            ...
            <div className="flex rounded-xl bg-slate-800 p-1 mb-5">
              {[
                ...
                <button key={t.id} onClick={() => setTab(t.id)}
                  className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${tab === t.id ? 'bg-slate-700 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}>
                  {t.label}
                </button>
              ))}
            </div>
            ...
            <div className="mb-4">
              ...
              <input
                ...
                placeholder={tab === 'create' ? 'Oyuncu 1' : 'Oyuncu 2'}
                value={playerName}
