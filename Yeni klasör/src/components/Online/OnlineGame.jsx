...
export default function OnlineGame({ gs, myPlayerNum, isMyTurn, onPointClick, onRestart, onLeave }) {
  ...
  return (
    <div className="full-screen flex flex-col overflow-hidden bg-slate-900">
      ...
      <div className="flex items-center justify-between px-3 pt-2 pb-0 shrink-0">
        <button onClick={onLeave} className="text-slate-500 active:text-slate-300 text-sm py-2 px-1">
          ← Çık
        </button>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-slate-700 border border-slate-800 rounded-full px-1.5 py-0.5">
            P{myPlayerNum}
          </span>
        </div>
      ...
      <div className="shrink-0">
        <ScoreBoard gs={gs} isAITurn={false} />
      </div>

      {/* Turn banner */}
      <div className="shrink-0">
        <TurnBanner gs={gs} isAITurn={false} lastScoredTriangles={lastScoredCount} />
      </div>
